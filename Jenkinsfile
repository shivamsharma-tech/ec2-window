            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: env.EC2_CREDENTIALS, keyFileVariable: 'KEY')]) {
                    sh """
                        ...
                        "powershell -NoProfile -Command \\"docker pull $DOCKER_IMAGE:$BUILD_NUMBER; docker stop ec2-window -ErrorAction SilentlyContinue; docker rm ec2-window -ErrorAction SilentlyContinue; docker run -d --name ec2-window -p 3000:3000 $DOCKER_IMAGE:$BUILD_NUMBER\\""
                    """
                }
            }
        }
